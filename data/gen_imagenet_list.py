...
if __name__ == '__main__':
    ...
    data_root = '../../../data/imagenet100_small'
    fout_train_label = open('./data/imagenet100small_train_label_%d_%.1f.txt'%(cls_num, ratio), 'w')
    fout_train_unlabel = open('./data/imagenet100small_train_unlabel_%d_%.1f.txt'%(cls_num, ratio), 'w')
    fout_test = open('./data/imagenet100small_test_%d_%.1f.txt'%(cls_num, ratio), 'w')
    train_data_root = os.path.join(data_root, 'train')
    test_data_root = os.path.join(data_root, 'val')

    train_folders = os.listdir(train_data_root)
    test_folders = os.listdir(test_data_root)

    for i, folder_name in enumerate(train_folders):
        files = []
        for extension in IMG_EXTENSIONS:
            files.extend(glob.glob(os.path.join(train_data_root, folder_name, '*' + extension) ))
    ...
    for i, folder_name in enumerate(test_folders):
        files = []
        for extension in IMG_EXTENSIONS:
            files.extend(glob.glob(os.path.join(test_data_root, folder_name, '*' + extension) ))
        for filename in files:
            fout_test.write('%s %d\n'%(filename[32:], i)) 
